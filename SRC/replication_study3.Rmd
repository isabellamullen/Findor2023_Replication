...
# Performs Welch’s ANOVA to test if these variables significantly differ across groups.
...
#Changes the baseline category from the default (likely "Control") to "Proportionality".
...
# s3personal  <- polr(E3 ~ group, data = s3, Hess=TRUE)
# 
# # Function to extract coefficients, standard errors, p-values, and odds ratios
# extract_polr_results <- function(model, baseline) {
...
#   # Compute odds ratios
#   results <- data.frame(
#     Estimate = round(ctable[, "Value"], 2),
#     `StdError` = round(ctable[, "Std. Error"], 2),
#     `p` = ifelse(p_values < 0.001, "<.001", round(p_values, 3)),
#     OR = round(exp(ctable[, "Value"]), 2),
#     `95CI` = paste0("[", round(exp(ci[, 1]), 2), ", ", round(exp(ci[, 2]), 2), "]")
#   )
#   
...
# 
# # Extract results
# personal_control <- extract_polr_results(s3personal, "Control")
# personal_reciprocity <- extract_polr_results(s3personal, "Reciprocity")
# 
# # Combine results
# table_data <- rbind(
#   cbind(Dependent = "Personal agreement", personal_control),
#   cbind(Dependent = "Personal agreement", personal_reciprocity)
# )
# colnames(table_data)
# 
# # Create table
# table_data %>%
#   select(Dependent, Baseline, Estimate, StdError, p, OR, X95CI) %>%
#   kable(caption = "Support for building social housing",
#         align = "lcccccc",
#         col.names = c("", "Baseline", "Estimate", "Std. Error", "p", "OR", "[95% CI]")) %>%
#   kable_styling(full_width = FALSE, position = "left") %>%
#   add_header_above(c(" " = 2, " " = 5)) %>%
#   pack_rows("Personal agreement", 1, 2, indent = TRUE) %>%
#   pack_rows("Majority perspective", 3, 4, indent = TRUE) %>%
#   footnote(general = "Ordered logit regression; OR=Odds Ratio; CI=95% Confidence Interval")
