...
```{r}
new_data <- data.frame(
  AGE = seq(min(s3$AGE), max(s3$AGE), length.out = 100),
  group = factor("Proportionality", levels = levels(s3$group)),   # Choose a valid level
  EDU = factor("Secondary (complete)", levels = levels(s3$EDU)),       # Choose a valid level
  SEX = factor("Female", levels = levels(s3$SEX)),          # Choose a valid level
  REG = factor("Bratislavsky", levels = levels(s3$REG))          # Choose a valid level
)

pred_probs <- predict(s3personal_probit, newdata = new_data, type = "probs")

pred_df <- as.data.frame(pred_probs)
pred_df$AGE <- new_data$AGE
pred_df <- tidyr::pivot_longer(pred_df, cols = -AGE, names_to = "Category",
                               values_to = "Probability")

ggplot(pred_df, aes(x = AGE, y = Probability, color = Category)) +
  geom_line(size = 1) +
  labs(title = "Effect of AGE on Predicted Probabilities",
       x = "AGE", y = "Probability") +
  theme_minimal()
```
